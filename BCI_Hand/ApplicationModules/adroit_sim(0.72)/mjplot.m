...
%
%  MJPLOT(DATA) plots a trajectory (sequence) with poses
%				given by the elements of the struct-array DATA. 
%
%  The plot is interactive:
%
%    Left mouse drag     rotate camera around look-at
%    Right mouse drag    translate in horizontal viewplane
%    Scroll              zoom in/out
%
...
%
%	 Arrow keys		 select geoms:
%						 left	- previous geom
%						 right	- next geom
%						 up		- select
%						 down	- deselct
%
%	 return/spacebar	 start / stop simulation, save DATA to workspace
...
%
%    G      geoms (cycle through solid/transparent/off)
%    0,1,2  geom visualization groups
%    I      inertias
%    T      tendons
%    S      sites
%    C      contact points
...
if nargin == 0
	...
	if ~isstruct(DATA)
		Q		= DATA;
		if size(Q,1) < model.nq
			error 'numeric input should have at least nq rows';
		end
		N		= size(Q,2);
		DATA	= mj('getdata');
		DATA	= repmat(DATA,[1 N]);
		for i=1:N
			mj('set','qpos',Q(1:model.nq,i));
			mj kinematics;
			DATA(i) = mj('getdata');
		end
	end
...
if any(enable.geom)
	...
	for i=1:model.ngeom,
		...
		if isnan(geom) || ~enable.geom(getappdata(geom,'visgroup')+1) || strcmp(get(geom,'visible'),'off')
			continue;
		end	
...
for i=2:model.nbody,
	...
	% update geom, skip world geoms for pose > 1
	set(inertia,'vertices', vert, 'visible', enable.inertia)
...
if model.nsite
	...
	for i=1:model.nsite
		...
		set(site,'vertices', vert, 'visible', enable.site)
...
if strcmp(enable.tendon,'on')
	if model.ntendon
		...
		wrap_xpos	= reshape(data.wrap_xpos',3,[])';			
		act			= data.act;

		for i=1:model.ntendon
			if model.wrap_type(ten_wrapadr(i)+1) ~= 1
...
if isfield(data,'contact')
	...
	for i=1: model.ncmax 
		if i <= length(contact)
			...
			vert	= vert + ones(size(vert,1),1)*pos;
			set(con,'vertices', vert,'tag','contact', 'visible', enable.contact)	
			setappdata(con, 'type', 'contact')
			setappdata(con, 'id', i)
			setappdata(con, 'pos', pos)
			setappdata(con, 'dim', contact(i).dim)
			setappdata(con, 'geom1', contact(i).geom1)
			setappdata(con, 'geom2', contact(i).geom2)
		else
...
switch evt.Key	
	...
	case 'g'	%  cycle off-flat-phong
		...
		if any(~enable.geom)
			enable.geom = [1 1 1];
			set(geoms, 'visible', 'on','facealpha',1);			
		elseif any(enable.geom == 1)
			enable.geom = [.3 .3 .3];
			set(geoms,'facealpha',.3);				
		else
			enable.geom = [0 0 0];
			set(geoms, 'visible', 'off');								
		end
		
	case {'0','1','2'}
		group = str2double(evt.Key);
		enable.geom(group+1) = 1-enable.geom(group+1);
		geoms = findobj(fig, 'tag', 'geom');
		for i = 1:length(geoms)
			if getappdata(geoms(i),'visgroup') == group
				set(geoms(i), 'visible', toggle(get(geoms(i),'visible')));
			end
		end
	...
	case 'slash'						% --- alternate mode 
		setappdata(fig,'alternate',true)%     (using 'slash' because of matlab limitations)
...
geoms	= handles.geoms;
ngeoms	= length(geoms);
i		= min(ngeoms,getappdata(fig,'selected_geom')); 
...
switch button
	...
	case 'extend'
		newrope		= false;
		if ~isempty(last_move_time) && toc(last_move_time) > 0.06			
			ropeUpdate(fig, newrope);		
		end
end
...
sph.vertices	= sph.vertices * scale * 0.003;

% === ground ===
groundhandle = makeGround(m);
...
if m.ntendon > 0
	...
	ten_type = m.wrap_type(m.tendon_adr+1);
	for i=1:m.ntendon	
		if ten_type(i) > 1
			tendonhandles(end+1) = patch('linewidth', 1.5,'tag', 'tendon', 'userdata', tendon_act(i),'xdata',nan, 'ydata',nan, 'zdata',nan,...
...
for i=1:m.ncmax
	...
	contactforcehandles(i)	= patch('edgecolor', [1 .1 .1], 'edgealpha',0.3, 'linewidth', 3.5, ...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% make ground plane
function groundhandle = makeGround(m)
...
tile_size	= m.geom_size(i,1)/m.geom_size(i,3);		% length of the side of a tile
nt			=  round(m.geom_size(i,3));		% number of tiles from the origin to the edge
...
for i=1:model.ngeom
	switch model.geom_type(i),
		...
		case 1,                 % heightfield
			nx		= model.hfield_ncol;
			ny		= model.hfield_nrow;
			hfsize  = model.geom_size(i,:);
			hfdata	= reshape(model.hfield_data, [nx ny])';
			[Vx,Vy] = meshgrid(linspace(-hfsize(1),hfsize(1),nx),linspace(-hfsize(2),hfsize(2),ny));
			[F,V]	= surf2patch(Vx, Vy, hfsize(3)*hfdata);
		case 2,                 % sphere
		...
		case 5,                 % cylinder
			...
			% close faces
			V(end+1,:) = [0 0 V(1,end)];	% lower center vertex
 			V(end+1,:) = [0 0 V(end,end)];	% upper center vertex
			lF			= mod(F,2).*F;
			lF(lF==0)	= max(F(:))+1;		% lower face
			uF			= (~mod(F,2)).*F;
			uF(uF==0)	= max(F(:))+2;		% upper face
			
			F = [F; lF; uF(:,[4 3 2 1])];	% fix orientation of upper face
			
		case 6,                 % box
			[F,V]		= makeBox(model.geom_size(i,:));			
		case 7,                % mesh
			id				= model.geom_dataid(i) + 1;
			F				= mesh.face(mesh.faceadr(id)+1 : mesh.faceadr(id)+mesh.facenum(id), :);
			F				= F + 1;
			V				= mesh.vert(mesh.vertadr(id) +(1: mesh.vertnum(id)), :);
	end
	...
	if ~isempty(F)
		...
		setappdata(geoms(i),'visgroup', model.geom_group(i))
...
% current point in pixels, (0,0) at top left
...
T = xform * [diag(scale) -offset; 0 0 0 1];

% normalized ray from camera to cursor
z	= pinv(T([1 2 4],:))*[cp 1]';
ray = z(1:3)'/z(4) - campos;
ray = ray/norm(ray);
ray = ray*sign(camdir*ray');
...
if newrope || isempty(rope)	% new cursor distance and rope length
	...
	t  =  ((anchor-campos)*camdir') / (ray*camdir');

	% get 3D location of cursor
	cursorpos = campos + t*ray;
	...
	cursorpos	= campos + ray*rope.dist;
end
...
minHoriz	= 1e-4;		 % minumum horizontal distance
