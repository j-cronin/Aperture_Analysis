function PlotDotsDirect(subject, trodeLocations, weights, viewSide, clims, markerSize, colormapName, trodeLabels, plotLabels)
...
%     colormap(cm);
    ...
    for chan=1:length(weights)
        ...
        if (isnan(weights(chan)))
            plot3(trodeLocations(chan,1), trodeLocations(chan,2), trodeLocations(chan,3),'.','Color',[0 0 0], 'MarkerSize', round(markerSize/2));
        else
            idx = idxs(chan);
