...
%     pos_shuffled = cell(1, length(trials));
    ...
    for i=9:9%trials
        ...
        %% Accuracy and chance accuracy:
        aper_accuracy = sum(data < high_boundary & data > low_boundary)/length(data);
        
        % Chance accuracy calculations:
        [pos_shuffled, entersTarget(i), chanceAccuracy(i), std_chanceAccuracy(i)] = aper_chanceAccuracy(data, data_interp, high_boundary, low_boundary);
        
        
        % Save
        save_path = strcat('C:\Users\jcronin\Box Sync\Lab\ECoG\Aperture\Data Analysis\', sid, '_accuracy');
        save('save_path', 'startTime', 'endTime', 'entersTarget', 'chanceAccuracy', 'std_chanceAccuracy');
    end
end